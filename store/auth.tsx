...
export const useAuth = create<AuthState>((set, get) => ({
  ...
  signUp: async (email, password, name) => {
    ...
    try {
      ...
      const response = await createUserWithEmailAndPassword(
        ...
        email,
        password,
      ); //Firebase feature for creating users by email and password
  ...
  googleAuth: async () => {
    try {
      ...
      const res = GoogleAuthProvider.credential(userInfo.idToken);
      const createdUser = await signInWithCredential(auth, res);
  ...
  isAuth: async (navigation: any) => {
    //We check here if phone has a token from user and is token not expitred
    try {
